export const arrow_curved_down_icon = `<svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 3.9687499 3.96875">
  <path stroke-width="0.318611" d="m 1.1578161,0.21695335 c -0.061702,-0.0329161 -0.81488983,-0.0133494 -0.81488983,-0.0133494 0,0 0.54265514,0.55608717 0.93827003,0.96284295 0.4433991,0.4558857 0.4777944,0.8151048 0.4777944,0.8151048 H 0.5130859 l 1.6505985,1.7274342 1.434036,-1.7247007 c -0.534692,-0.00674 -0.7326378,0.00633 -1.0334189,0.013349 0,0 -0.1208928,-1.09485632 -1.4064854,-1.78068085 z" />
</svg>`;

export const arrow_curved_up_icon = `<svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 3.9687499 3.96875">
  <path stroke-width="0.318611" d="m 2.7893699,3.7809852 c 0.061702,0.032916 0.8148899,0.013349 0.8148899,0.013349 0,0 -0.5426551,-0.5560871 -0.9382701,-0.9628429 C 2.2225906,2.375606 2.1881953,2.0163869 2.1881953,2.0163869 H 3.4341002 L 1.7835016,0.28895256 0.34946563,2.0136534 c 0.534692,0.00674 0.73263777,-0.00633 1.03341887,-0.013349 0,0 0.1208928,1.0948563 1.4064854,1.7806808 z" />
</svg>
`;

export const arrow_filter_down_hover_icon = `<svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 5.2916664 5.2916666">
  <path stroke-width="0.259554" d="M0.3363627,2.5225803H4.9507688L2.6460125,5.030853Z" />
</svg>
`;

export const arrow_filter_down_icon = `<svg xmlns="http://www.w3.org/2000/svg" fill="rgba(255, 255, 100, 0.5)" viewBox="0 0 5.2916664 5.2916666">
  <path stroke-width="0.259554" d="M0.3363627,2.5225803H4.9507688L2.6460125,5.030853Z" />
</svg>`;

export const arrow_filter_up_down_icon = `<svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 5.2916664 7.0555555">
  <path stroke-width="0.26" d="M0.3363627,3.8317165H4.9507688L2.6460125,6.3399892Z" />
  <path stroke-width="0.26" d="M4.9518209,3.0005711H0.33741474L2.6421711,0.4922984Z" />
</svg>
`;

export const arrow_filter_up_hover_icon = `<svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 5.2916664 5.2916666">
  <path stroke-width="0.259554" d="M4.9503442,5.0927726H0.33593811L2.6406944,2.5844999Z" />
</svg>
`;

export const arrow_filter_up_icon = `<svg xmlns="http://www.w3.org/2000/svg" fill="rgba(255, 255, 100, 0.5)" viewBox="0 0 5.2916664 5.2916666" >
  <path stroke-width="0.26" d="M4.9503442,2.7500013H0.3359381L2.6406944,0.24172863Z"/>
</svg>
`;

export const arrow_invert_down_icon = `<svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 16 16">
  <rect width="16" height="16" fill="transparent" stroke="none" />
  <path d="m 7.9099511,3.5888063 0.01,9.8499997 m 0,0 L 10.80995,9.7888064 M 7.9199511,13.438806 5.1999503,9.7888064 H 10.82995" stroke-linecap="round" stroke-linejoin="round" />
</svg>`;

export const arrow_invert_up_icon = `<svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 16 16">
  <rect width="16" height="16" fill="transparent" stroke="none" />
  <path d="m 8.1199994,13.438806 -0.01,-9.8499999 m 0,0 L 5.22,7.2388059 M 8.1099994,3.5888061 10.83,7.2388059 H 5.2" stroke-linecap="round" stroke-linejoin="round" />
</svg>
`;

export const arrow_left_icon = `<svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 4.63 3.31">
  <path stroke-width="0.28" d="M4.37,2.06V1.26H2.59V0.51L0.49,1.68 2.59,2.81V2.05Z" />
</svg>
`;

export const arrow_right_icon = `<svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 4.63 3.31">
  <path stroke-width="0.28" d="M0.33,1.26V2.06H2.11V2.80L4.21,1.64 2.11,0.51V1.27Z" />
</svg>
`;

export const arrows_right_left_icon = `<svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 5.2916667 5.2916667">
  <path stroke-width="0.279868" d="M1.0611627,1.2579603v0.7960966h1.7729567v0.745551L4.9415095,1.6370539 2.8387006,0.50609109V1.2674376Z" />
  <path stroke-width="0.279868" d="M4.2096487,4.0871794V3.2910826H2.436692V2.5455317L0.32930199,3.7080856 2.4321108,4.8390485V4.077702Z" />
</svg>
`;

export const palette_icon = `<svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24">
  <path stroke="currentColor" stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M12 7h.01m3.486 1.513h.01m-6.978 0h.01M6.99 12H7m9 4h2.706a1.957 1.957 0 0 0 1.883-1.325A9 9 0 1 0 3.043 12.89 9.1 9.1 0 0 0 8.2 20.1a8.62 8.62 0 0 0 3.769.9 2.013 2.013 0 0 0 2.03-2v-.857A2.036 2.036 0 0 1 16 16Z"/>
</svg>`;

export const shape_circle_icon = `<svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 100 100">
  <circle cx="50" cy="50" r="50" />
</svg>`;

export const shape_square_icon = `<svg xmlns="http://www.w3.org/2000/svg" width="100" height="100" viewBox="0 0 100 100">
  <polygon points="100,100 0,100 0,0 100,0" />
</svg>`;

export const shape_triangle_icon = `<svg xmlns="http://www.w3.org/2000/svg" width="100" height="100" viewBox="0 0 100 100">
  <polygon points="100,100 0,100 50,0" />
</svg>`;