<script lang="ts">
  ...
  onMount(() => {
    ...
    else panelsSizesStore.set(getDefaultPanelSizes(6));
...
<Splitpanes horizontal dblClickSplitter={false} theme="modern-theme" on:resize={startResize} on:resized={resizeRow}>
  ...
  <Pane minSize={20}>
    <Splitpanes dblClickSplitter={false} theme="modern-theme" on:resize={startResize} on:resized={resizeLower}>
      ...
      >
      <Pane class="panel-{panels[5].id}" minSize={getPanelMinSize(panels[5].id)} size={panelsSizes[5]}
        ><Panel otherPanels={panels} {handleSwap} currPanel={panels[5]} /></Pane
      >
    </Splitpanes>
  </Pane>
</Splitpanes>
